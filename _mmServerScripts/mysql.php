...
define('mysqli_NOT_EXISTS', create_error("Your PHP server doesn't have the MySQL module loaded or you can't use the mysqli_(p)connect functions."));
...
*/
	...
	function GetTables($table_name = '')
	{
		...
		if ($this->isOpen && isset($this->connectionId) && is_resource($this->connectionId)){
			...
			$results = mysqli_query($conn,$sql, $this->connectionId) or $this->HandleException();
	...
	function GetColumnsOfTable($TableName)
	{
		...
		$result = mysqli_query($conn,$query) or $this->HandleException();
	...
	function ExecuteSQL($aStatement, $MaxRows)
	{
		...
		$result = mysqli_query($conn,$aStatement) or $this->HandleException();
